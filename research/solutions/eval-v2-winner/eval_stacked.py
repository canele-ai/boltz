"""Evaluator for stacked optimizations: ODE + TF32 + bf16 trunk.

Runs multiple configurations against the eval-v2 baseline (torch 2.6.0 +
cuequivariance kernels). Each configuration is a combination of:
- ODE sampling (gamma_0=0)
- TF32 matmul precision
- bf16 trunk (no .float() upcast)

All configurations run on L40S with cuequivariance kernels enabled.

Usage:
    # Single config
    modal run research/solutions/eval-v2-winner/eval_stacked.py --mode eval \
        --config '{"sampling_steps": 20, "recycling_steps": 0, "gamma_0": 0.0}'

    # Full sweep with 3 runs each
    modal run research/solutions/eval-v2-winner/eval_stacked.py --mode sweep --validate

    # Sanity check
    modal run research/solutions/eval-v2-winner/eval_stacked.py --mode sanity
"""
...
boltz_image = (
    ...
    .add_local_file(
        str(ORBIT_DIR / "boltz_wrapper_stacked.py"),
        remote_path="/eval/boltz_wrapper_stacked.py",
    )
)

app = modal.App("boltz-eval-stacked-v2", image=boltz_image)
...
def evaluate_config(config_json: str, num_runs: int = 1) -> str:
    ...
    for tc in test_cases:
        ...
        for run_idx in range(num_runs):
            ...
            print(
                f"[eval-stacked] Running {tc_name} run {run_idx + 1}/{num_runs} "
            ...
            if pred_result["error"] is not None:
                ...
                print(f"[eval-stacked] ERROR: {last_error[:500]}")
                break

            if pred_result["wall_time_s"] is not None:
                run_times.append(pred_result["wall_time_s"])
                print(f"[eval-stacked] {tc_name} run {run_idx + 1}: "
...
def sanity_check() -> str:
    ...
    wrapper_path = "/eval/boltz_wrapper_stacked.py"
...
def main(
    ...
    """Stacked optimizations evaluation harness.
    ...
    Usage:
        modal run research/solutions/eval-v2-winner/eval_stacked.py --mode sanity
        modal run research/solutions/eval-v2-winner/eval_stacked.py --mode eval --config '{"sampling_steps": 20, "recycling_steps": 0, "gamma_0": 0.0}'
        modal run research/solutions/eval-v2-winner/eval_stacked.py --mode eval --config-name ODE-20/0r+TF32 --validate
        modal run research/solutions/eval-v2-winner/eval_stacked.py --mode sweep --validate
    """
    if validate:
        num_runs = 3
    ...
    if mode == "sanity":
        print("[eval-stacked] Running sanity check...")
        ...
        if config_name and config_name in SWEEP_CONFIGS:
            ...
            cfg = SWEEP_CONFIGS["ODE-20/0r"]

        print(f"[eval-stacked] Evaluating: {json.dumps(cfg)} (num_runs={num_runs})")
        ...
        print(f"[eval-stacked] Running sweep of {len(SWEEP_CONFIGS)} configs (num_runs={num_runs})")

        # Launch all configs in parallel using .map()
...
def _print_summary(result: dict):
    ...
    for pc in result.get("per_complex", []):
        if pc.get("error"):
            ...
            print(f"  {pc['name']}: {t:.1f}s, pLDDT={p:.4f}" + (f" [{times_str}]" if times_str else ""))
