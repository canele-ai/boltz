"""Stacked-ultra evaluator: all compatible optimizations combined.

Measures each optimization incrementally on eval-v3 (cached MSAs, GPU-only timing).
Single model load, process all complexes sequentially.

Optimizations stacked:
1. Single model load (eliminates per-complex model load overhead)
2. ODE-20/0r (gamma_0=0, 20 steps, recycling=0)
3. TF32 matmul precision ("high")
4. bf16 trunk (remove .float() upcast in triangular_mult)
5. SDPA attention (torch.nn.functional.scaled_dot_product_attention)
6. torch.compile on score model (mode="reduce-overhead")

Usage:
    # Run full incremental sweep (3 seeds each)
    modal run orbits/stacked-ultra/eval_stacked_ultra.py --mode sweep

    # Run a single named config
    modal run orbits/stacked-ultra/eval_stacked_ultra.py --mode eval --config-name "ODE+TF32+bf16+SDPA"

    # Run custom config
    modal run orbits/stacked-ultra/eval_stacked_ultra.py --mode eval --config '{"sdpa": true, "compile_score": true}'
"""
...
app = modal.App("boltz-eval-stacked-ultra", image=boltz_image)

msa_volume = modal.Volume.from_name("boltz-msa-cache-v3", create_if_missing=True)

# ---------------------------------------------------------------------------
# Incremental configurations (each builds on the previous)
# ---------------------------------------------------------------------------
...
def _patch_attention_sdpa():
    """Replace AttentionPairBias.forward with SDPA in both attention.py and attentionv2.py."""
    import torch
...
# ---------------------------------------------------------------------------
# MSA caching (copied from eval-v3 evaluator)
# ---------------------------------------------------------------------------
...
def _parse_confidence_from_dir(results_dir):
    """Parse confidence metrics from a results directory."""
    quality = {}
...
# ---------------------------------------------------------------------------
# Modal function: single model load, all test cases
# ---------------------------------------------------------------------------
...
def evaluate_stacked(config_json: str) -> str:
    """Run all test cases with a SINGLE model load + stacked optimizations."""
    import torch
    ...
    # Apply matmul precision BEFORE any model imports
    ...
    patches_applied = [f"gamma_0={gamma_0}"]

    if config.get("bf16_trunk", False):
        _patch_triangular_mult_bf16()
        patches_applied.append("bf16_trunk")

    if config.get("sdpa", False):
        _patch_attention_sdpa()
        patches_applied.append("sdpa")

    print(f"[stacked-ultra] Patches: {', '.join(patches_applied)}")
    print(f"[stacked-ultra] matmul_precision={config.get('matmul_precision', 'highest')}")
    print(f"[stacked-ultra] compile_score={config.get('compile_score', False)}")

    use_kernels = config.get("enable_kernels", True)
    try:
        import cuequivariance_torch
        print(f"[stacked-ultra] cuequivariance_torch: {cuequivariance_torch.__version__}")
    except ImportError:
        use_kernels = False
        print("[stacked-ultra] cuequivariance_torch NOT available")
    ...
    print(f"[stacked-ultra] Download/cache: {download_time:.1f}s")
    ...
    compile_score = config.get("compile_score", False)
    ...
    # --- Warmup for compile ---
    if compile_score:
        print("[stacked-ultra] torch.compile warmup (first run will be slow)...")
    ...
    if use_msa_cache:
        print("[stacked-ultra] Using pre-cached MSAs (eval-v3)")
    else:
        print("[stacked-ultra] WARNING: No MSA cache - times will include MSA server latency")
    ...
    for tc in test_cases:
        ...
        print(f"[stacked-ultra] Running {tc_name}")

        try:
            t_start = time.perf_counter()

            data = boltz_main.check_inputs(effective_yaml)
            out_dir = work_dir / f"boltz_results_{effective_yaml.stem}"
            out_dir.mkdir(parents=True, exist_ok=True)

            boltz_main.process_inputs(
                data=data, out_dir=out_dir, ccd_path=cache / "ccd.pkl",
                mol_dir=mol_dir, use_msa_server=not use_msa_cache,
            ...
            print(f"[stacked-ultra] {tc_name}: total={wall_time:.1f}s, gpu={gpu_time:.1f}s, "
            ...
            print(f"[stacked-ultra] {tc_name}: ERROR - {exc}")
    ...
    if successful:
        ...
        results["aggregate"]["mean_time_with_amortized_load"] = mean_inference + load_time / n
        if mean_gpu:
        ...
        if baseline_time:
            ...
            results["aggregate"]["speedup_with_amortized_load"] = baseline_time / (mean_inference + load_time / n)
            if mean_gpu:
...
def main(
    ...
    """Stacked-ultra evaluation.

    Modes:
        eval   - Run single config (default: ODE+TF32+bf16+SDPA)
        multi  - Run single config with 3 seeds
        sweep  - Run ALL incremental configs with 3 seeds each
    """
    if mode == "eval":
        if config_name and config_name in INCREMENTAL_CONFIGS:
            cfg = dict(INCREMENTAL_CONFIGS[config_name])
        elif config:
            cfg = json.loads(config)
        else:
            cfg = dict(INCREMENTAL_CONFIGS["ODE+TF32+bf16+SDPA"])

        if seed >= 0:
            cfg["seed"] = seed

        print(f"[stacked-ultra] Evaluating: {json.dumps(cfg)}")
        result_json = evaluate_stacked.remote(json.dumps(cfg))
        ...
        seeds = [42, 123, 7]
        if config_name and config_name in INCREMENTAL_CONFIGS:
            cfg_base = dict(INCREMENTAL_CONFIGS[config_name])
        elif config:
            cfg_base = json.loads(config)
        else:
            cfg_base = dict(INCREMENTAL_CONFIGS["ODE+TF32+bf16+SDPA"])
        ...
        print(f"[stacked-ultra] Running {len(seeds)} seeds in parallel: {seeds}")
        all_results = []
        for s, result_json in zip(seeds, evaluate_stacked.map(config_jsons)):
            result = json.loads(result_json)
            all_results.append(result)
            agg = result.get("aggregate", {})
            print(f"  Seed {s}: speedup={agg.get('speedup', 0):.2f}x, "
                  f"pLDDT={agg.get('mean_plddt', 0):.4f}, "
                  f"load={result.get('load_time_s', 0):.1f}s")

        _print_multi_summary(all_results, seeds, config_name or "custom")

    elif mode == "sweep":
        seeds = [42, 123, 7]
        config_names = list(INCREMENTAL_CONFIGS.keys())

        print(f"[stacked-ultra] Incremental sweep: {len(config_names)} configs x {len(seeds)} seeds")

        all_sweep_results = {}

        for cfg_name in config_names:
            cfg_base = dict(INCREMENTAL_CONFIGS[cfg_name])
            config_jsons = []
            for s in seeds:
                c = dict(cfg_base)
                c["seed"] = s
            ...
            all_sweep_results[cfg_name] = seed_results

        # Print incremental comparison table
        print(f"\n{'='*70}")
        print("INCREMENTAL STACKING RESULTS")
        print(f"{'='*70}")
        print(f"{'Config':<30} {'Speedup':>10} {'pLDDT':>8} {'Delta':>8} {'Gate':>6}")
        print("-" * 70)

        for cfg_name in config_names:
            seed_results = all_sweep_results[cfg_name]
            speedups = [r["aggregate"].get("speedup", 0) for r in seed_results]
            plddts = [r["aggregate"].get("mean_plddt", 0) for r in seed_results]
            deltas = [r["aggregate"].get("plddt_delta_pp", 0) for r in seed_results if "plddt_delta_pp" in r.get("aggregate", {})]
            gates = [r["aggregate"].get("passes_quality_gate", False) for r in seed_results]

            mean_s = sum(speedups) / len(speedups) if speedups else 0
            std_s = _std(speedups)
            mean_p = sum(plddts) / len(plddts) if plddts else 0
            mean_d = sum(deltas) / len(deltas) if deltas else 0
            all_pass = all(gates) if gates else False

            print(f"{cfg_name:<30} {mean_s:.2f}x+/-{std_s:.2f} {mean_p:.4f} {mean_d:+.1f}pp {'PASS' if all_pass else 'FAIL':>6}")

        # Per-complex breakdown for best config
        best_name = config_names[-1]
        print(f"\n--- Per-complex breakdown ({best_name}) ---")
        for r in all_sweep_results[best_name]:
            s = r["config"].get("seed", "?")
            for pc in r.get("per_complex", []):
                if pc.get("error"):
                    print(f"  Seed {s} / {pc['name']}: ERROR")
                else:
                    t = pc.get("wall_time_s", 0)
                    g = pc.get("gpu_time_s", 0)
                    p = pc.get("quality", {}).get("complex_plddt", "N/A")
                    print(f"  Seed {s} / {pc['name']}: total={t:.1f}s, gpu={g:.1f}s, pLDDT={p}")

        print("\n--- FULL RESULTS ---")
        print(json.dumps(all_sweep_results, indent=2))
...
def _print_summary(result):
    ...
    for key, label in [
        ("mean_wall_time_s", "Mean time"),
        ("mean_time_no_load", "Mean time (no load)"),
        ("mean_time_with_amortized_load", "Mean time (amortized load)"),
        ("mean_gpu_time", "Mean GPU time"),
        ("mean_plddt", "Mean pLDDT"),
        ("speedup", "Speedup"),
        ("speedup_no_load", "Speedup (no load)"),
        ("speedup_with_amortized_load", "Speedup (amortized load)"),
    ...
    for pc in result.get("per_complex", []):
        if pc.get("error"):
            ...
            tstr = f"{t:.1f}s" if t else "N/A"
            gstr = f"{g:.1f}s" if g else "N/A"
            pstr = f"{p:.4f}" if p else "N/A"
            print(f"  {pc['name']}: total={tstr}, gpu={gstr}, pLDDT={pstr}")


def _print_multi_summary(all_results, seeds, config_name):
    def safe_mean(vals):
        return sum(vals) / len(vals) if vals else 0

    speedups = [r["aggregate"].get("speedup", 0) for r in all_results]
    plddts = [r["aggregate"].get("mean_plddt", 0) for r in all_results]
    loads = [r.get("load_time_s", 0) for r in all_results]

    print(f"\n{'='*60}")
    print(f"MULTI-SEED SUMMARY: {config_name} (n={len(seeds)})")
    print(f"{'='*60}")
    print(f"  Speedup:      {safe_mean(speedups):.2f}x +/- {_std(speedups):.2f}")
    print(f"  Mean pLDDT:   {safe_mean(plddts):.4f}")
    print(f"  Model load:   {safe_mean(loads):.1f}s")

    # Per-complex
    print(f"\n{'Complex':<20} ", end="")
    for s in seeds:
        print(f"{'Seed '+str(s):>16} ", end="")
    print()
    print("-" * (20 + 17 * len(seeds)))

    for tc_idx in range(3):
        tc_names = ["small_complex", "medium_complex", "large_complex"]
        if tc_idx >= len(tc_names):
            break
        print(f"{tc_names[tc_idx]:<20} ", end="")
        for r in all_results:
            pcs = r.get("per_complex", [])
            if tc_idx < len(pcs) and pcs[tc_idx].get("wall_time_s"):
                t = pcs[tc_idx]["wall_time_s"]
                print(f"{t:>13.1f}s   ", end="")
            else:
                print(f"{'ERR':>16} ", end="")
        print()
