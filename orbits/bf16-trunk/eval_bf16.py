"""Evaluator for BF16 trunk experiment.

Runs Boltz-2 evaluation with the bf16 trunk patch (removing fp32 upcast
in triangular multiply) combined with ODE-20/0r config from orbit/ode-sampler.

Tests both quality (does bf16 trunk maintain pLDDT?) and timing (is it faster?).

Usage:
    # BF16 triangular multiply + ODE-20/0r (primary experiment)
    modal run orbits/bf16-trunk/eval_bf16.py --config '{"sampling_steps": 20, "recycling_steps": 0, "gamma_0": 0.0, "bf16_trunk": true}'

    # BF16 tri mult + outer product mean
    modal run orbits/bf16-trunk/eval_bf16.py --config '{"sampling_steps": 20, "recycling_steps": 0, "gamma_0": 0.0, "bf16_trunk": true, "bf16_opm": true}'

    # Control: fp32 trunk + ODE-20/0r (to compare timing)
    modal run orbits/bf16-trunk/eval_bf16.py --config '{"sampling_steps": 20, "recycling_steps": 0, "gamma_0": 0.0}'

    # Validate (3 runs)
    modal run orbits/bf16-trunk/eval_bf16.py --config '{"sampling_steps": 20, "recycling_steps": 0, "gamma_0": 0.0, "bf16_trunk": true}' --validate
"""
...
boltz_image = (
    ...
    .add_local_file(str(ORBIT_DIR / "boltz_wrapper_bf16.py"), remote_path="/eval/boltz_wrapper_bf16.py")
)

app = modal.App("boltz-eval-bf16-trunk", image=boltz_image)
...
def _run_boltz_prediction(
    ...
    """Run a single Boltz-2 prediction with bf16 trunk support."""
    wrapper = str(Path("/eval/boltz_wrapper_bf16.py"))
    cmd = [
...
def evaluate(config_json: str, num_runs: int = 1) -> str:
    ...
    for tc in test_cases:
        ...
        for run_idx in range(num_runs):
            ...
            bf16_flags = []
            if merged.get("bf16_trunk"):
                bf16_flags.append("tri_mult")
            if merged.get("bf16_opm"):
                bf16_flags.append("opm")

            print(
                f"[eval-bf16] Running {tc_name} run {run_idx + 1}/{num_runs} "
                f"steps={merged['sampling_steps']}, recycle={merged['recycling_steps']}, "
                f"gamma_0={merged.get('gamma_0', 0.8)}, bf16={bf16_flags or 'none'}"
            )
...
def main(
    ...
    """BF16 trunk evaluation harness.

    Usage:
        # BF16 tri mult + ODE-20/0r
        modal run orbits/bf16-trunk/eval_bf16.py --config '{"sampling_steps": 20, "recycling_steps": 0, "gamma_0": 0.0, "bf16_trunk": true}'

        # With validation (3 runs)
        modal run orbits/bf16-trunk/eval_bf16.py --config '{"sampling_steps": 20, "recycling_steps": 0, "gamma_0": 0.0, "bf16_trunk": true}' --validate
    """
    if validate:
        num_runs = 3
    ...
    print(f"[eval-bf16] Evaluating config: {json.dumps(cfg)} (num_runs={num_runs})")
    ...
    if speedup is not None:
        print(f"\n[eval-bf16] Speedup: {speedup:.2f}x")
    if plddt_delta is not None:
        print(f"[eval-bf16] pLDDT delta: {plddt_delta:+.2f} pp")
    if passes is not None:
        status = "PASS" if passes else "FAIL"
        print(f"[eval-bf16] Quality gate: {status}")
