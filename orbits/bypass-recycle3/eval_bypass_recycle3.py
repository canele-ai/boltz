"""Evaluator for bypass-lightning with recycling_steps=3 on eval-v5.

Config E validation: bypass Lightning Trainer + ODE-12 + TF32 + bf16 + recycling=3.
Measures wall time, predict-only time, pLDDT, and CA RMSD against PDB ground truth.

Uses eval-v5 test cases (1BRS, 1DQJ, 2DN2) with pre-cached MSAs (boltz-msa-cache-v5)
and ground truth structures (boltz-ground-truth-v1).
...
# Use eval-v5 test cases and config from main repo
...
app = modal.App("boltz-eval-bypass-recycle3", image=boltz_image)
...
TEST_CONFIG: dict[str, Any] = {
    "sampling_steps": 12,
    "recycling_steps": 3,   # <-- the point of this experiment
...
def _run_boltz_bypass(
    ...
    """Run prediction using bypass-lightning wrapper."""
...
# ---------------------------------------------------------------------------
# Modal function: evaluate one (test_case, seed) pair
# ---------------------------------------------------------------------------
...
def evaluate_one(tc_name: str, seed: int) -> str:
    """Evaluate a single test case with a single seed.

    Returns JSON with wall_time_s, predict_only_s, pLDDT, CA RMSD.
    """
    import yaml
    ...
    if not tc_yaml.exists():
        return json.dumps({"tc_name": tc_name, "seed": seed,
                           "error": f"YAML not found: {tc_yaml}"})

    work_dir = Path(f"/tmp/boltz_eval/{tc_name}_s{seed}_{uuid.uuid4().hex[:8]}")
    ...
    print(f"[eval] Running {tc_name} seed={seed} "
    ...
    if pred_result["error"] is None:
        struct_result = _compare_structures(work_dir, effective_yaml, gt_root, tc_name)
        pred_result["structural"] = struct_result
        if "ca_rmsd" in struct_result:
            print(f"[eval] {tc_name} seed={seed}: CA RMSD = {struct_result['ca_rmsd']}A")
        elif "error" in struct_result:
            print(f"[eval] {tc_name} seed={seed}: structural comparison error: {struct_result['error']}")

    pred_result["tc_name"] = tc_name
    pred_result["seed"] = seed

    if pred_result["error"] is None:
        print(f"[eval] {tc_name} seed={seed}: wall={pred_result['wall_time_s']:.1f}s "
...
def main():
    """Run bypass-recycle3 evaluation: 3 test cases x 3 seeds = 9 jobs in parallel."""
    import statistics

    test_cases = ["small_complex", "medium_complex", "large_complex"]

    # Build all (tc, seed) pairs
    jobs = [(tc, seed) for tc in test_cases for seed in SEEDS]
    tc_args = [j[0] for j in jobs]
    seed_args = [j[1] for j in jobs]

    print(f"[eval] Launching {len(jobs)} parallel evaluations on Modal L40S...")
    print(f"[eval] Config: {json.dumps(TEST_CONFIG, indent=2)}")
    print(f"[eval] Seeds: {SEEDS}")

    # Run all 9 jobs in parallel via .map()
    results_raw = list(evaluate_one.map(tc_args, seed_args))
    ...
    print("\n" + "=" * 80)
    print("BYPASS-RECYCLE3 EVALUATION RESULTS (eval-v5)")
    print(f"Config: ODE-12/3r + TF32 + bf16 + bypass + warmup")
    print("=" * 80)
    ...
    for tc in test_cases:
        tc_results = by_tc.get(tc, [])
        errors = [r for r in tc_results if r.get("error")]
        if errors:
            print(f"\n{tc}: ERRORS")
            for e in errors:
                print(f"  seed={e['seed']}: {e['error'][:200]}")
            continue

        wall_times = [r["wall_time_s"] for r in tc_results]
        predict_times = [r.get("predict_only_s") for r in tc_results if r.get("predict_only_s")]
        plddts = [r["quality"]["complex_plddt"] for r in tc_results if "complex_plddt" in r.get("quality", {})]
        ...
        if mean_wall:
            total_wall_times.extend(wall_times)
        if predict_times:
            total_predict_times.extend(predict_times)
        if plddts:
            total_plddts.extend(plddts)
        if ca_rmsds:
            total_ca_rmsds.extend(ca_rmsds)

        print(f"\n--- {tc} ---")
        print(f"  {'Seed':<8} {'Wall(s)':<10} {'Predict(s)':<12} {'pLDDT':<10} {'CA RMSD':<10}")
        for r in tc_results:
            s = r["seed"]
        ...
        if mean_wall:
            ...
            print(f"  Baseline: wall={bl['wall_time_s']:.1f}s pLDDT={bl['plddt']:.3f} "
                  f"CA_RMSD={bl['ca_rmsd']:.3f}A")
            if mean_rmsd is not None:
                rmsd_delta = mean_rmsd - bl["ca_rmsd"]
                print(f"  CA RMSD delta: {rmsd_delta:+.3f}A vs baseline")

    # --- Aggregate ---
    if total_wall_times:
        # Per-complex mean wall time, then average across complexes
        per_tc_means = []
        for tc in test_cases:
            tc_walls = [r["wall_time_s"] for r in by_tc.get(tc, [])
                        if r.get("wall_time_s") and not r.get("error")]
            if tc_walls:
                per_tc_means.append(statistics.mean(tc_walls))

        mean_wall_all = statistics.mean(per_tc_means) if per_tc_means else None
        mean_plddt_all = statistics.mean(total_plddts) if total_plddts else None
        ...
        if mean_wall_all:
            ...
            print(f"  Mean wall time: {mean_wall_all:.1f}s (baseline: {baseline_mean_wall:.1f}s)")
            print(f"  Speedup: {speedup:.2f}x")
        if total_predict_times:
            print(f"  Mean predict-only: {statistics.mean(total_predict_times):.1f}s")
        if mean_plddt_all:
            print(f"  Mean pLDDT: {mean_plddt_all:.4f} (baseline: 0.9650)")
        if total_ca_rmsds:
            # Per-complex mean RMSD
            for tc in test_cases:
                tc_rmsds = [r["structural"]["ca_rmsd"] for r in by_tc.get(tc, [])
                            if "ca_rmsd" in r.get("structural", {})]
                if tc_rmsds:
                    bl_rmsd = eval_v5_baseline[tc]["ca_rmsd"]
                    print(f"  {tc} CA RMSD: {statistics.mean(tc_rmsds):.3f}A "
                          f"(baseline: {bl_rmsd:.3f}A)")
