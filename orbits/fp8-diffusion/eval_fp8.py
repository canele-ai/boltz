"""Evaluator for fp8-diffusion orbit: bf16 attention on top of eval-v2-winner.

Builds on the eval-v2-winner stack (ODE-20, recycling=0, TF32+bf16 trunk)
and adds bf16 attention (removes .float() upcast in AttentionPairBias).

Configurations tested:
- baseline-stack: ODE-20/0r + TF32 + bf16 trunk (eval-v2-winner reference)
- bf16-attn: + bf16 attention in score model and trunk
- bf16-full: + bf16 pairformer sequence stack

Usage:
    modal run orbits/fp8-diffusion/eval_fp8.py --mode sanity
    modal run orbits/fp8-diffusion/eval_fp8.py --mode eval --config-name bf16-attn
    modal run orbits/fp8-diffusion/eval_fp8.py --mode sweep --validate
"""
...
# eval dir is at the repo root (two levels up from orbits/fp8-diffusion/)
...
boltz_image = (
    ...
    .add_local_file(
        str(ORBIT_DIR / "boltz_wrapper_fp8.py"),
        remote_path="/eval/boltz_wrapper_fp8.py",
    )
)

app = modal.App("boltz-eval-fp8-diffusion", image=boltz_image)
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "bf16_pairformer": False,
}

# Configurations to sweep -- incremental bf16 additions
SWEEP_CONFIGS = {
    # Reference: eval-v2-winner stack (for comparison)
    "baseline-stack": {
        "sampling_steps": 20,
        "recycling_steps": 0,
        "gamma_0": 0.0,
    ...
    # New: add bf16 attention (score model + trunk attention)
    "bf16-attn": {
        "sampling_steps": 20,
        "recycling_steps": 0,
        "gamma_0": 0.0,
    ...
    # New: add bf16 pairformer sequence stack on top
    "bf16-full": {
        "sampling_steps": 20,
        "recycling_steps": 0,
        "gamma_0": 0.0,
...
def _run_boltz_prediction(
    ...
    """Run a single Boltz-2 prediction with the fp8 wrapper."""
    wrapper = str(Path("/eval/boltz_wrapper_fp8.py"))
    cmd = [
    ...
    if config.get("bf16_attention", False):
        cmd.append("--bf16_attention")
    if config.get("bf16_pairformer", False):
        cmd.append("--bf16_pairformer")
...
def _compute_aggregates(results: dict, eval_config: dict) -> dict:
    ...
    if invalid_plddts:
        agg["error"] = f"Invalid pLDDT values: {invalid_plddts}"
...
def evaluate_config(config_json: str, num_runs: int = 1) -> str:
    ...
    if use_msa_cache:
        print("[eval] MSA cache detected -- using pre-cached MSAs")
    else:
        print("[eval] No MSA cache -- using MSA server")
    ...
    for tc in test_cases:
        ...
        for run_idx in range(num_runs):
            ...
            print(
                ...
                f"bf16_attn={merged.get('bf16_attention', False)}, "
                f"bf16_pf={merged.get('bf16_pairformer', False)}"
            )
...
def sanity_check() -> str:
    ...
    wrapper_path = "/eval/boltz_wrapper_fp8.py"
    ...
    # Test that monkey-patching works
    try:
        spec = importlib.util.spec_from_file_location("wrapper_test", wrapper_path)
        mod = importlib.util.module_from_spec(spec)
        # Don't execute -- just verify it parses
        results["wrapper_parseable"] = True
    except Exception as e:
        results["wrapper_parseable"] = False
        results["wrapper_error"] = str(e)
...
def main(
    ...
    """FP8-diffusion orbit evaluation harness.
    ...
    Usage:
        modal run orbits/fp8-diffusion/eval_fp8.py --mode sanity
        modal run orbits/fp8-diffusion/eval_fp8.py --mode eval --config-name bf16-attn
        modal run orbits/fp8-diffusion/eval_fp8.py --mode eval --config-name bf16-attn --validate
        modal run orbits/fp8-diffusion/eval_fp8.py --mode sweep --validate
    """
    if validate:
        num_runs = 3
    ...
    if mode == "sanity":
        print("[eval-fp8] Running sanity check...")
        ...
        if config_name and config_name in SWEEP_CONFIGS:
            ...
            cfg = SWEEP_CONFIGS["bf16-attn"]

        print(f"[eval-fp8] Evaluating: {json.dumps(cfg)} (num_runs={num_runs})")
        ...
        print(f"[eval-fp8] Running sweep of {len(SWEEP_CONFIGS)} configs (num_runs={num_runs})")
