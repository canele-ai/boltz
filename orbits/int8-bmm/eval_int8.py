"""Evaluator for INT8 weight-only quantization with BMM triangle mul.

Runs end-to-end Boltz-2 predictions with:
- BMM triangle multiplication (bypasses cuequivariance)
- torchao INT8 weight-only quantization on all Linear layers
- Stacked on ODE-12 + TF32 + bf16

Configurations:
- bmm-only: BMM replacement, no INT8 (baseline for this orbit)
- int8-bmm: BMM + INT8 weight-only quantization

Usage:
    modal run orbits/int8-bmm/eval_int8.py --mode sanity
    modal run orbits/int8-bmm/eval_int8.py --mode eval --config-name int8-bmm
    modal run orbits/int8-bmm/eval_int8.py --mode sweep
"""
...
boltz_image = (
    ...
    .add_local_file(
        str(ORBIT_DIR / "boltz_wrapper_int8.py"),
        remote_path="/eval/boltz_wrapper_int8.py",
    )
)

app = modal.App("boltz-eval-int8-bmm", image=boltz_image)
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "bf16_trunk": False,
    "apply_int8": False,
    "int8_mode": "weight_only",
    "skip_patterns": "",
}

SWEEP_CONFIGS = {
    # BMM only (no INT8) - baseline comparison for this orbit
    "ODE12-TF32-bf16-bmm": {
        "sampling_steps": 12,
        "recycling_steps": 0,
        "gamma_0": 0.0,
    ...
    # INT8 weight-only (W8A16) + BMM + ODE12 + TF32 + bf16
    "ODE12-TF32-bf16-int8wo-bmm": {
        "sampling_steps": 12,
        "recycling_steps": 0,
        "gamma_0": 0.0,
        "matmul_precision": "high",
    ...
    # INT8 dynamic (W8A8) + BMM + ODE12 + TF32 + bf16
    "ODE12-TF32-bf16-int8dyn-bmm": {
        "sampling_steps": 12,
        "recycling_steps": 0,
        "gamma_0": 0.0,
        "matmul_precision": "high",
...
def _run_boltz_prediction(input_yaml, out_dir, config):
    """Run a single Boltz-2 prediction with the INT8 wrapper."""
    wrapper = str(Path("/eval/boltz_wrapper_int8.py"))
    cmd = [
    ...
    if config.get("apply_int8"):
        cmd.append("--apply_int8")
        int8_mode = config.get("int8_mode", "weight_only")
        cmd.extend(["--int8_mode", int8_mode])

    skip = config.get("skip_patterns", "")
    if skip:
        cmd.extend(["--skip_patterns", skip])
...
def evaluate_config(config_json: str, num_runs: int = 1) -> str:
    ...
    try:
        import torchao
        results["env"]["torchao_version"] = torchao.__version__
    except ImportError:
        results["env"]["torchao_version"] = None
    ...
    if use_msa_cache:
        print("[eval-int8] MSA cache detected")
    else:
        print("[eval-int8] No MSA cache")
    ...
    for tc in test_cases:
        ...
        for run_idx in range(num_runs):
            ...
            desc = (
                ...
                f"int8={run_config.get('apply_int8', False)}"
            )
            print(f"[eval-int8] Running {tc_name} run {run_idx + 1}/{num_runs} ({desc})")
...
def sanity_check() -> str:
    """Verify environment: torch, torchao, boltz, INT8 quantization on a small model."""
    import torch
    ...
    try:
        import torchao
        results["torchao_version"] = torchao.__version__
    except ImportError:
        results["torchao_version"] = None
        results["torchao_error"] = "NOT INSTALLED"
    ...
    # Test INT8 quantization on a small model
    if results.get("torchao_version"):
        try:
            ...
            results["int8_test"] = {
                "max_abs_error": err,
                "pass": err < 1.0,  # INT8 weight-only has some error
            }
        except Exception as e:
            results["int8_test"] = {"error": str(e)}

    # Test that cuequivariance is NOT installed (we don't need it)
    try:
        import cuequivariance_torch
        results["cuequivariance"] = "INSTALLED (unexpected)"
    except ImportError:
        results["cuequivariance"] = "not installed (expected)"
...
def main(
    ...
    """INT8 BMM evaluation harness.

    Modes:
        sanity  - Verify environment and INT8 quantization
        eval    - Run single config
        sweep   - Run all predefined configurations

    Usage:
        modal run orbits/int8-bmm/eval_int8.py --mode sanity
        modal run orbits/int8-bmm/eval_int8.py --mode eval --config-name ODE12-TF32-bf16-int8-bmm
        modal run orbits/int8-bmm/eval_int8.py --mode sweep --validate
    """
    if validate:
        num_runs = 3

    if mode == "sanity":
        print("[eval-int8] Running sanity check...")
        ...
        if config_name and config_name in SWEEP_CONFIGS:
            ...
            cfg = SWEEP_CONFIGS["ODE12-TF32-bf16-int8dyn-bmm"]

        print(f"[eval-int8] Evaluating: {json.dumps(cfg)} (num_runs={num_runs})")
        ...
        print(f"[eval-int8] Running sweep of {len(SWEEP_CONFIGS)} configs (num_runs={num_runs})")
