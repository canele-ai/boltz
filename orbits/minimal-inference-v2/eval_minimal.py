"""Minimal Inference Pipeline v2: per-phase timing with proper MSA caching.

Fixes the bug in v1 where `process_inputs(..., use_msa_server=True)` hit the
ColabFold MSA server on every call, adding ~5-8s of network latency per complex.
Now uses pre-cached MSAs from Modal volume `boltz-msa-cache-v3` to eliminate
this confound, matching the eval-v3 harness approach.
...
Usage:
    modal run orbits/minimal-inference-v2/eval_minimal.py --mode eval
    modal run orbits/minimal-inference-v2/eval_minimal.py --mode multi
"""
...
app = modal.App("boltz-eval-minimal-inference-v2", image=boltz_image)

# Persistent volume for pre-cached MSA files (eval-v3).
# Generated by: modal run research/eval/generate_msa_cache.py
...
def _inject_cached_msas(
    ...
    print(f"[minimal-v2] MSA cache: injected {injected} cached MSA(s) for {target_name}")
    return cached_yaml
...
def evaluate_minimal(config_json: str) -> str:
    ...
    if config.get("bf16_trunk", False):
        ...
        print("[minimal-v2] bf16 trunk patch applied")
    ...
    if use_msa_cache:
        print("[minimal-v2] MSA cache detected — using pre-cached MSAs (eval-v3)")
    else:
        print("[minimal-v2] WARNING: No MSA cache — falling back to MSA server (adds network latency)")
    ...
    results["msa_cached"] = use_msa_cache
    ...
    for tc in test_cases:
        ...
        print(f"[minimal-v2] Running {tc_name}")
        ...
        try:
            ...
            msa_cached_for_tc = False
            if use_msa_cache:
                cached_yaml = _inject_cached_msas(tc_yaml, msa_cache_root, work_dir)
                if cached_yaml is not None:
                    effective_yaml = cached_yaml
                    msa_cached_for_tc = True
                else:
                    print(f"[minimal-v2] WARNING: No cached MSAs for {tc_name}, falling back to server")
            ...
            boltz_main.process_inputs(
                ...
                mol_dir=mol_dir,
                use_msa_server=not msa_cached_for_tc,
