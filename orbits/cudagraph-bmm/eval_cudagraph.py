"""Evaluator for CUDA graph capture via torch.compile with bmm path.

Runs end-to-end Boltz-2 predictions comparing:
- bmm-only (no compile, baseline from triton-pairformer)
- bmm + torch.compile(mode="reduce-overhead") on Pairformer
- bmm + torch.compile(mode="default") on Pairformer
- cuequivariance baseline

All configs stack ODE-12/0r + TF32 + bf16.

Usage:
    modal run orbits/cudagraph-bmm/eval_cudagraph.py --mode sanity
    modal run orbits/cudagraph-bmm/eval_cudagraph.py --mode eval --config-name bmm-compile-pf
    modal run orbits/cudagraph-bmm/eval_cudagraph.py --mode sweep
"""
...
# Also need triton-pairformer for the triton_triangle_mul module (dependency)
...
app = modal.App("boltz-eval-cudagraph-bmm", image=boltz_image)
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    # wrapper selection: "cudagraph" uses our wrapper, "triton" uses parent's
    "wrapper": "cudagraph",
}

SWEEP_CONFIGS = {
    # Baseline: ODE-12 + TF32 + bf16 + cuequivariance (current best from eval-v2-winner)
    "baseline-cueq": {
        "sampling_steps": 12,
        "recycling_steps": 0,
        "gamma_0": 0.0,
        ...
        "bf16_trunk": True,
        "wrapper": "triton",
        # triton wrapper flags for cueq baseline
        "_triton_flags": ["--no_triton", "--enable_kernels"],
    },
    # bmm-only (no compile) -- matches triton-pairformer matmul config
    "bmm-only": {
        "sampling_steps": 12,
        "recycling_steps": 0,
        "gamma_0": 0.0,
    ...
    # bmm + torch.compile(mode="reduce-overhead") on Pairformer -- CUDA graph capture
    "bmm-compile-pf-reduce": {
        "sampling_steps": 12,
        "recycling_steps": 0,
        "gamma_0": 0.0,
        "matmul_precision": "high",
    ...
    # bmm + torch.compile(mode="default") on Pairformer -- Inductor without CUDA graphs
    "bmm-compile-pf-default": {
        "sampling_steps": 12,
        "recycling_steps": 0,
        "gamma_0": 0.0,
        "matmul_precision": "high",
    ...
    # bmm + torch.compile(mode="max-autotune") -- maximum optimization
    "bmm-compile-pf-autotune": {
        "sampling_steps": 12,
        "recycling_steps": 0,
        "gamma_0": 0.0,
        "matmul_precision": "high",
...
def _run_boltz_prediction(input_yaml, out_dir, config):
    ...
    wrapper_type = config.get("wrapper", "cudagraph")
    ...
    if wrapper_type == "triton":
        ...
        # Add triton-specific flags
        for flag in config.get("_triton_flags", []):
            cmd.append(flag)
    else:
        ...
        if config.get("compile_score"):
            cmd.append("--compile_score")
        if config.get("compile_mode"):
            cmd.extend(["--compile_mode", config["compile_mode"]])
        if config.get("compile_dynamic"):
    ...
    try:
        ...
        # Capture compile-related output
        if "CUDA" in (proc.stderr or "") or "graph" in (proc.stderr or ""):
            result["compile_notes"] = proc.stderr[-1000:]
...
def evaluate_config(config_json: str, num_runs: int = 1) -> str:
    ...
    if use_msa_cache:
        print("[eval-cudagraph] MSA cache detected")
    else:
        print("[eval-cudagraph] No MSA cache")
    ...
    for tc in test_cases:
        ...
        for run_idx in range(num_runs):
            ...
            desc = (
                ...
                f"bmm={run_config.get('use_bmm', False)}, "
                f"compile_pf={run_config.get('compile_pairformer', False)}, "
                f"mode={run_config.get('compile_mode', 'N/A')}"
            )
            print(f"[eval-cudagraph] Running {tc_name} run {run_idx + 1}/{num_runs} ({desc})")
...
def sanity_check() -> str:
    """Verify environment: torch, CUDA, compile capability."""
    import torch
    results = {
        "torch_version": torch.__version__,
    ...
    try:
        ...
        results["compile_reduce_overhead"] = "OK"
    except Exception as e:
        results["compile_reduce_overhead"] = f"FAIL: {e}"

    # Test bmm replacement correctness
    try:
        B, N, K, D = 1, 64, 64, 32
        a = torch.randn(B, N, K, D, device="cuda")
        b = torch.randn(B, N, K, D, device="cuda")
        ...
        results["bmm_correctness"] = {"error": str(e)}
...
def main(
    ...
    """CUDA graph + bmm evaluation harness.
    ...
    Usage:
        modal run orbits/cudagraph-bmm/eval_cudagraph.py --mode sanity
        modal run orbits/cudagraph-bmm/eval_cudagraph.py --mode eval --config-name bmm-compile-pf-reduce
        modal run orbits/cudagraph-bmm/eval_cudagraph.py --mode sweep
    """
    if validate:
        num_runs = 3
    ...
    if mode == "sanity":
        ...
        if config_name and config_name in SWEEP_CONFIGS:
            ...
            cfg = SWEEP_CONFIGS["bmm-compile-pf-reduce"]
