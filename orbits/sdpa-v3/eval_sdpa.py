"""Evaluator for SDPA attention replacement stacked with ODE-20/0r + TF32 + bf16.

Uses eval-v3 infrastructure with pre-cached MSAs for reproducible GPU-only timing.

Configurations tested:
1. eval-v2-winner baseline: ODE-20/0r + TF32 + bf16 (no SDPA)
2. SDPA variant: same as above + SDPA attention replacement

Both configs run with 3 seeds for statistical significance.

Usage:
    # Sanity check
    modal run orbits/sdpa-v3/eval_sdpa.py --mode sanity

    # Single eval (1 run, fast iteration)
    modal run orbits/sdpa-v3/eval_sdpa.py --mode eval --config-name sdpa

    # Full validation (3 runs per config)
    modal run orbits/sdpa-v3/eval_sdpa.py --mode sweep --validate

    # Compare SDPA vs no-SDPA
    modal run orbits/sdpa-v3/eval_sdpa.py --mode compare --validate
"""
...
app = modal.App("boltz-eval-sdpa-v3", image=boltz_image)

# Pre-cached MSA volume (eval-v3)
...
def _inject_cached_msas(input_yaml: Path, msa_cache_root: Path, work_dir: Path):
    """Create modified input YAML with msa: fields for cached MSAs."""
    import yaml
    import shutil
...
def evaluate_config(config_json: str, num_runs: int = 1) -> str:
    """Run evaluation for a single configuration with cached MSAs."""
    import statistics
    ...
    for tc in test_cases:
        ...
        for run_idx in range(num_runs):
            ...
            print(
                ...
                f"gamma_0={merged.get('gamma_0')}, TF32={merged.get('matmul_precision') == 'high'}, "
                f"bf16_trunk={merged.get('bf16_trunk')}, sdpa={merged.get('sdpa')}"
            )
...
def sanity_check() -> str:
    ...
    if torch.cuda.is_available():
        try:
            # Quick test to see which backends are available
            from torch.backends.cuda import (
                flash_sdp_enabled,
                mem_efficient_sdp_enabled,
                math_sdp_enabled,
            )
            results["sdpa_backends"] = {
                "flash": flash_sdp_enabled(),
                "mem_efficient": mem_efficient_sdp_enabled(),
                "math": math_sdp_enabled(),
            }
        except ImportError:
            results["sdpa_backends"] = "could not check"

    # MSA cache check
    msa_cache_root = Path("/msa_cache")
    if msa_cache_root.exists():
        cached = list(msa_cache_root.iterdir())
        results["msa_cache_entries"] = len(cached)
        results["msa_cache_names"] = [d.name for d in cached if d.is_dir()]
    else:
...
def main(
    ...
    Modes:
        ...
        sweep   - Run all configs
        compare - Run control + SDPA side-by-side
    """
    if validate:
        num_runs = 3
    ...
    if mode == "sanity":
        ...
        print(f"[eval-sdpa] Running {len(config_names)} configs (num_runs={num_runs})")
        ...
        # Head-to-head SDPA difference
        if "control" in all_results and "sdpa" in all_results:
            ctrl_time = all_results["control"]["aggregate"].get("mean_wall_time_s")
            sdpa_time = all_results["sdpa"]["aggregate"].get("mean_wall_time_s")
            if ctrl_time and sdpa_time:
                diff_pct = (ctrl_time - sdpa_time) / ctrl_time * 100
                relative = ctrl_time / sdpa_time
                print(f"\nSDPA vs Control: {relative:.3f}x ({diff_pct:+.1f}%)")
                print(f"  Control: {ctrl_time:.1f}s, SDPA: {sdpa_time:.1f}s")
...
def _print_summary(result: dict):
    ...
    for pc in result.get("per_complex", []):
        if pc.get("error"):
            ...
            print(f"  {pc['name']}: {t_str}, pLDDT={p_str}" +
